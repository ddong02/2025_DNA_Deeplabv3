...
def get_dataset(opts):
    ...
    # 증강 파라미터 설정 (WandB config에서 가져오거나 기본값 사용)
    ...
    rotation_limit = getattr(opts, 'rotation_limit', 10)
...
def main():
    ...
    if opts.enable_vis:
        ...
        if run_name is None:
            # 증강 파라미터를 포함한 동적 run name 생성
            flip_p = getattr(opts, 'horizontal_flip_p', 0.5)
            brightness = getattr(opts, 'brightness_limit', 0.2)
            contrast = getattr(opts, 'contrast_limit', 0.2)
            rotation = getattr(opts, 'rotation_limit', 10)
            
            run_name = f"aug_flip{flip_p:.1f}_br{brightness:.2f}_ct{contrast:.2f}_rot{rotation}"
        ...
        # 증강 파라미터 로깅
        if hasattr(opts, 'horizontal_flip_p'):
            # WandB에 증강 파라미터를 메트릭으로 로깅
            vis.vis_table("Augmentation Parameters", {
                ...
                "Rotation Limit": f"±{getattr(opts, 'rotation_limit', 10)}°"
            })
            
            print("📊 Augmentation parameters logged to WandB (table)")
    else:
