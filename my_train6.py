...
class DNA2025Dataset(Dataset):
    def __init__(self, root_dir, crop_size, subset, scale_range, val_ratio=0.2, random_seed=1):
        self.crop_size = crop_size
        ...
        # Load and split data from training set only
        train_base = os.path.join(root_dir, "SemanticDataset_final")
        self.image_paths = []
        self.label_paths = []
        
        # Load from cam0~cam5 and set2 folders with train/val split
        cam_folders = ['cam0', 'cam1', 'cam2', 'cam3', 'cam4', 'cam5', 'set2']
        ...
        for cam in cam_folders:
            ...
            # Calculate split point for this camera folder
            ...
            if subset == 'train':
                # Use the majority for training
                selected_indices = indices[num_val:]
            elif subset == 'val':
                # Use the minority for validation
                selected_indices = indices[:num_val]
            else:
                raise ValueError(f"Unknown subset: {subset}. Use 'train' or 'val'")
            ...
            print(f"  {cam}: {len(cam_images)} total -> {len(selected_indices)} for {subset}")
        
        # 데이터 검증
        assert len(self.image_paths) == len(self.label_paths), f"Image/label count mismatch: {len(self.image_paths)} vs {len(self.label_paths)}"
        assert len(self.image_paths) > 0, f"No images found for {subset} subset"
        ...
        print(f"Loaded {len(self.image_paths)} {subset} images (val_ratio={val_ratio})")
...
def get_dataset(opts):
    ...
    if opts.dataset == 'voc':
        ...
        # Training mode: use train/val split from training data
