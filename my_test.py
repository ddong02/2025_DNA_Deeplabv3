...
def evaluate_custom_miou(pred_dir, label_dir, num_classes):
    ...
    for pred_path in tqdm(pred_paths, desc="Computing confusion matrix"):
        ...
        if "_leftImg8bit.png" in filename:
            ...
            file_id = filename.replace("_comparison.png", "")
        else:
    ...
    with torch.no_grad():
        for i, (images, labels) in tqdm(enumerate(loader), total=len(loader)):
            ...
            # Debug: Check the range of predictions and targets
            if i == 0:  # Print for first batch only
                print(f"Debug - Prediction range: {preds.min()}-{preds.max()}")
                print(f"Debug - Target range: {targets.min()}-{targets.max()}")
            ...
            if opts.save_results and opts.save_samples:
                for batch_idx in range(len(images)):
                    ...
                    # Create comparison image only (원본 | 정답 | 예측)
...
def main():
    ...
    # Setup metrics (use our own implementation)
    ...
    # Custom mIoU evaluation은 comparison 이미지로는 불가능하므로 제거
    # (comparison 이미지는 시각화용이므로 실제 예측 마스크가 아님)
